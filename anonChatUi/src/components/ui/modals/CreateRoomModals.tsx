...
const CreateRoomModals = ({
  ...
  async function handleRoomCreation(e: FormEvent<HTMLFormElement>) {
    ...
    const roomId = uuidv4();
    console.log("Creating room with:", { username, roomUrl: `chat/${roomId}`, title, topics });
  ...
  return (
    <Modal isOpen={isModalShowing} setIsOpen={setIsModalShowing} type={"input"}>
      <div className="p-8">
        {/* TITLE */}
        ...
        <form className="space-y-6" onSubmit={(e) => handleRoomCreation(e)}>
          <div>
            <label className="block text-gray-400 text-xs font-mono uppercase mb-2" htmlFor="username">
              User Name (required)
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-gray-400 text-xs font-mono uppercase mb-2" htmlFor="roomTitle">
